class MessageBuilder
  ...
  def bark_about_old_pull_requests
    ...
    "AAAAAAARGH! #{these(old_pull_requests.length)} #{pr_plural(old_pull_requests.length)} not been updated in over 2 days.\n\n#{angry_bark.join}\nRemember each time you forget to review your pull requests, a baby seal gets vigorously petted to the point that it doesn't seem quite so cute anymore.
    \n\n#{informative_bark}"
  end
  ...
  def present(pull_request, index)
    ...
    approved = pr["approved"] ? " | :yay: Yay, it's been APPROVED. Ship it! :yay: " : ""
    <<-EOF.gsub(/^\s+/, '')
